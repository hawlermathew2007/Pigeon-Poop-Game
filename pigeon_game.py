import pygame
import random
# import suorlg	#problem here 		#can work individually but cant work when combine together 
# Details suorlg: it stands for Sign up or Login, it's a success program, but i found it annoying and unnecessary so I remove it from the program
import os
import linecache
...
basket = pygame.image.load(f"{images_path}/basket.png").convert()
branch = pygame.image.load(f"{images_path}/branch.png").convert()
pigeon = pygame.image.load(f"{images_path}/pigeon.png").convert()
button_coin = pygame.image.load(f"{images_path}/coin_button.png").convert()
button_velocity = pygame.image.load(f"{images_path}/velocity.png").convert()		# the image break down! # solution: make the image small in the figma
button_life = pygame.image.load(f"{images_path}/life.png").convert()
prize = pygame.image.load(f"{images_path}/prize.png").convert()
coin = pygame.image.load(f"{images_path}/coin.png")
life_heart = pygame.image.load(f"{images_path}/life_heart.png").convert()
...
if not os.path.exists("highscore_pi.txt"):
	...
	with open("highscore_pi.txt", "r") as f:
		highscore = int(linecache.getline("highscore_pi.txt", 1))
		# user = linecache.getline("highscore_pi.txt", 2)
...
class P_poop():
	...
	def draw(self):
		pygame.draw.rect(screen,brown,self.rect,border_radius=10)
...
with open("highscore_pi.txt", "w") as f:	#change here
	f.write(str(highscore))
# with open("highscore_pi.txt", "a") as f:
# 	f.write(str(highscore) + "\n")
# 	f.write(str(user))

# f = open("player_information.txt", "r")
# data = f.readlines()
# for i in range(len(data)):
# 	print(("Username: " + str(user)))
# 	print(data[i])
# 	if ("Username: " + str(user)) == data[i]:
# 		data[i + 3] = "Highscore: " + str(highscore) + "\n"
# f = open("player_information.txt", "w")
# f.writelines(data)
# f.close()

pygame.quit()

#can import here bruh sheshshhh
